...
const ACCESS_TOKEN = 'SLACK_accessTOKEN';
const request = {
  body: {
...
function setup({ accessToken, mapTeamToAccessToken, verificationToken } = {
    accessToken: ACCESS_TOKEN,
    mapTeamToAccessToken: jest.fn(),
  ...
  return {
    connector: new SlackConnector({ accessToken, mapTeamToAccessToken, verificationToken }),
...
describe('#createContext', () => {
  ...
  it('should create SlackContext and has customAccessToken', async () => {
    const mapTeamToAccessToken = jest.fn().mockResolvedValue('anyToken');
    const { connector } = setup({ mapTeamToAccessToken });
    const event = {
      rawEvent: {
        recipient: {
          id: 'anyPageId',
  ...
  it('should call warning if it could not find teamId', async () => {
    const mapTeamToAccessToken = jest.fn().mockResolvedValue('anyToken');
    const { connector } = setup({ mapTeamToAccessToken });
    const event = {
      rawEvent: {},
    ...
    expect(warning).toBeCalledWith(
      false,
      'Could not find teamId from request body.'
    );
  });
});
